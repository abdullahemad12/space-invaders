...
;; main
;; Game -> Game
;; starts the space invaders game by evaluating G0
...
;; update-state
;; Game -> Game
;; updates all the next position of the missles and invaders and creates new invaders
...
;; removes the shot invaders from the list
...
;; template rules used from ListOfInvader data defintion, added ListOfMissile to the parameters
...
;; checks whether the given invader is shot
...
;; template rules used from ListOfMissiles data defintion, added invader to the parameters
...
;; move-invaders
;; updates the position of the invaders and randomly adds invaders to the list
...
;; template rules used from the data definiton of ListOfInvader
...
;; Randomly chooses to add an invador to an empty list or not, then returns the list
...
;; templates rules used from the Invader data defintion
...
;; template used from the Invader data definition
...
;; template rules used from ListOfMissile data definition
...
;;template rules used from tank data definition
...
;; render-game
;; Game -> Image
;; draws all the components of game on the screen
...
;; template rules used from game data definition
...
;; places the invadors over the given image
...
;; template used from data definition of ListOfInvader
...
;; template rules taken from invader data defintion
...
;; template rules used from ListOfMissiles data definition, added an image parameter 
...
(check-expect (handle-key G3 " ") (make-game (list I1 I2)
                                                 (list M1 M2 (make-missile (tank-x T1) (- HEIGHT (/ (image-height TANK)2))))
                                                 T1))
...
;; templates rules used from Invader data defintion
